...
gulp.task('scripts', function() {
    return gulp.src([ // Все необходимые библиотеки
        ...
        'app/libs/svg4everybody/dist/svg4everybody.min.js' //SVG for Everybody adds SVG External Content support to all browsers
        ])
        .pipe(plumber())
...
gulp.task('icoSync', function () {
    return gulp.src('')
        .pipe(plumber())
        .pipe(dirSync('app/icons', 'dist/icons',{printSummary: true}))
...
gulp.task('build', ['clean','minify','img','sass', 'scripts'], function() {
    ...
    var buildIcon = gulp.src('dist/icons/sprite/*.svg') // Переносим иконки в продакшен
...
gulp.task('svgSpriteBuild', function () {
    return gulp.src('app/icons/icons/*.svg')
    // minify svg
        .pipe(svgmin({
            js2svg: {
                pretty: true
